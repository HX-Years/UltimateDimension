...
import ultimatedimension.ui.HeatImage;
import ultimatedimension.ui.PayloadImage;
import ultimatedimension.ui.PowerImage;
...
public class MultiCrafter extends PayloadBlock {
  ...
  public void init() {
    ...
    if (resolvedRecipes == null && recipes != null)
      resolvedRecipes = parser.parse(this, recipes);
    if (resolvedRecipes == null || resolvedRecipes.isEmpty())
      throw new ArcRuntimeException(MultiCrafterParser.genName(this)
          + " has no recipe! It's perhaps because all recipes didn't find items, fluids or payloads they need. Check your `last_log.txt` to obtain more information.");
    if (switchStyle == null)
      switchStyle = RecipeSwitchStyle.get(menu);
    decorateRecipes();
    setupBlockByRecipes();
